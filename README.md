...
- **OpenAPI 3.1 contract**: [`src/http/openapi.yaml`](src/http/openapi.yaml) is the single source of truth — request/response types, runtime validation, and the Redoc reference docs (served at `/docs`) are all driven from it. CI gates contract drift via `redocly lint`.
...
Start the HTTP server to accept capture requests via JSON over HTTP. The Redoc-rendered reference docs are served at `/docs`, and the raw spec at `/openapi.yaml` for tooling.
...
See [docs/http-api.md](docs/http-api.md) for curl examples and request/response details. Reference docs (Redoc, generated by `redocly build-docs`) are served at <http://localhost:8080/docs>; the raw spec is at <http://localhost:8080/openapi.yaml>.
...
- `npm run openapi:build-docs` — render `dist/docs/index.html` (self-contained Redoc HTML with the spec embedded; runs as part of `prebuild`)
- CI: [`.github/workflows/openapi.yml`](.github/workflows/openapi.yml) re-runs `redocly lint` on every PR with GitHub Actions annotations.

The pre-rendered `dist/docs/index.html` is what the server serves at `/docs`; restart the server to pick up spec edits.
