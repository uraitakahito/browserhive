 ...
 * `./bin/up.sh`, which writes `.e2e-stack.json`. This setup reads that file
 * and probes the running server. If the stack is not reachable it throws,
 * so a forgotten `bin/up.sh` fails loudly instead of silently skipping.
 *
...
export default async function setup({ provide }: GlobalSetupApi): Promise<void> {
  ...
  try {
    ...
    throw new Error(".e2e-stack.json not found — bring the stack up first: ./bin/up.sh");
  }
  ...
  const reachable = await fetch(`${endpoints.api}/v1/status`)
    ...
    .catch(() => false);
  if (!reachable) {
    throw new Error(`E2E stack not reachable at ${endpoints.api}. Run ./bin/up.sh first.`);
  }
