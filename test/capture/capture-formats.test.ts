...
describe("validateCaptureFormats", () => {
  it("should return valid when all formats are enabled", () => {
    const formats: CaptureFormats = { png: true, jpeg: true, html: true, links: false, pdf: false };
    const result = validateCaptureFormats(formats);
    expect(result.ok).toBe(true);
  });

  it("should return valid when only png is enabled", () => {
    const formats: CaptureFormats = { png: true, jpeg: false, html: false, links: false, pdf: false };
    const result = validateCaptureFormats(formats);
    expect(result.ok).toBe(true);
  });

  it("should return valid when only jpeg is enabled", () => {
    const formats: CaptureFormats = { png: false, jpeg: true, html: false, links: false, pdf: false };
    const result = validateCaptureFormats(formats);
    expect(result.ok).toBe(true);
  });

  it("should return valid when only html is enabled", () => {
    const formats: CaptureFormats = { png: false, jpeg: false, html: true, links: false, pdf: false };
    const result = validateCaptureFormats(formats);
    expect(result.ok).toBe(true);
  });

  it("should return valid when png and jpeg are enabled", () => {
    const formats: CaptureFormats = { png: true, jpeg: true, html: false, links: false, pdf: false };
    const result = validateCaptureFormats(formats);
    expect(result.ok).toBe(true);
  });

  it("should return valid when only links is enabled", () => {
    const formats: CaptureFormats = { png: false, jpeg: false, html: false, links: true, pdf: false };
    const result = validateCaptureFormats(formats);
    expect(result.ok).toBe(true);
  });

  it("should return valid when only pdf is enabled", () => {
    const formats: CaptureFormats = { png: false, jpeg: false, html: false, links: false, pdf: true };
    const result = validateCaptureFormats(formats);
    expect(result.ok).toBe(true);
  });

  it("should return invalid when all formats are disabled", () => {
    const formats: CaptureFormats = { png: false, jpeg: false, html: false, links: false, pdf: false };
    const result = validateCaptureFormats(formats);
