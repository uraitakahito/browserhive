  ...
  describe("captureWorkerMachine", () => {
    ...
    describe("operational state", () => {
      ...
      it("should transition idle → processing on TASK_STARTED and capture currentTask", async () => {
        const { actor } = await createOperationalActor();
        const task = { taskId: "t1", labels: [], url: "https://example.com", retryCount: 0, captureFormats: { png: true, jpeg: false, html: false, links: false }, dismissBanners: false, enqueuedAt: "2024-01-01T00:00:00.000Z" };
      ...
      it("should mark task complete on TASK_FAILED when retries exhausted", async () => {
        const taskQueue = new TaskQueue();
        const { actor } = await createOperationalActor({ runtime: { taskQueue } });
        const task = { taskId: "t1", labels: ["test"], url: "https://example.com", retryCount: 2, captureFormats: { png: true, jpeg: false, html: false, links: false }, dismissBanners: false, enqueuedAt: "2024-01-01T00:00:00.000Z" };
    ...
    describe("error history cap", () => {
      it("should keep at most 10 error records", async () => {
        ...
        for (let i = 0; i < 12; i++) {
          const task = { taskId: `t${String(i)}`, labels: [], url: "https://example.com", retryCount: 2, captureFormats: { png: true, jpeg: false, html: false, links: false }, dismissBanners: false, enqueuedAt: "2024-01-01T00:00:00.000Z" };
          const result = {
            task,
            status: "failed" as const,
