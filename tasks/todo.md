...
- Phase 1 commit: `a1616ef` — `local` 系 (LocalArtifactStore / LocalStorageConfig / DEFAULT_STORAGE_CONFIG / `--storage` / `--output-dir` / `BROWSERHIVE_STORAGE` / `BROWSERHIVE_OUTPUT_DIR`) を全削除、`StorageConfig` から `kind` 判別子除去、page-capturer 系 5 テストを `FakeArtifactStore` 経由に移行。tests 467 → グリーン。
- Phase 2 commit: `76f1d6c` — `compose.dev.s3.yaml` overlay を削除し MinIO + minio-init を `compose.dev.yaml` / `compose.prod.yaml` 双方に統合。prod の MinIO は `expose:` のみ（host port 非公開）。`Dockerfile.prod` から `mkdir -p /app/output` を削除。
- Phase 3 commit: 本コミット — README の Storage セクション縮約、env 表行削除（`--storage` / `--output-dir` / 関連 env）、Mermaid 図の `Files` ノード → `Storage[(MinIO / S3)]` ノード差し替え、TLS 例の `--output-dir` を `--s3-*` に置換、`docker run` standalone 例を S3 env ベースに更新、レビュー欄記入。
- スモーク結果:
  - **dev**: `compose.dev.yaml up -d --build` → MinIO 健康・bucket `browserhive` 作成・BrowserHive 起動 (`/v1/status` 200, 2 workers operational)。`POST /v1/captures` で example.com を PNG+HTML キャプチャ → `s3://browserhive/<taskId>_smoke.png` (17 KiB) と `<taskId>_smoke.html` (596 B) を `mc ls` で確認。サーバログの `storage` 構造は `accessKeyId: "mini***dmin"` / `secretAccessKey: "***"` でマスク動作。
  - **prod**: `compose.prod.yaml up -d --build` → 同様に MinIO + minio-init 健康、`/v1/status` 200、`POST /v1/captures` で `s3://browserhive/<taskId>_prod-smoke.png` (16 KiB) と `<taskId>_prod-smoke.html` (596 B) 保存を確認。`down -v` で `browserhive-minio-prod-data` 含めクリーンアップ。
- テスト総数: 27 files / 467 tests（PoC 環境差分なし、全部グリーン）。
- 想定リスク振り返り:
  - `examples/data-client.ts` は HTTP のみで storage 非依存 → 変更なし、テストも通過。
  - CI の `safe-chain` は `@aws-sdk/client-s3` 既存追加済みで依存増減なし。
  - 旧 `./output` ホストディレクトリは compose では未参照になるが、ファイル自体は残る（実 prod では存在しないことが多い、運用者が手動破棄で OK）。
  - prod 同梱 MinIO の `minio-data` ボリュームバックアップ運用 → README "Production Environment" の Note に明示。
