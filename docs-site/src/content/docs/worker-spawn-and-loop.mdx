...
Complete — remove from `processing` and tally the outcome:
...
| `succeeded` / `failed` | Counters, not sets | incremented by markComplete |

Finished tasks leave **only a count**. They used to accumulate in a
`Set<string>` of task ids that nothing ever removed from, so it grew for as
long as the server ran. The result itself now goes to a `CaptureResultSink`,
which is what `GET /v1/captures/{taskId}` and the `.result.json` manifest
read from — see [Capture results](/capture-results/).

The two counters are deliberately separate: `markComplete` is called both
when a capture succeeded and when the retry budget ran out, so a single
"completed" number would mean "no longer our problem" rather than "worked".

`/v1/status` reports the queue sizes, both counters, plus `peekPending` (the
