...
```sh
./bin/up.sh 2        # SeaweedFS + worker×2 + browserhive:prod
./bin/status.sh      # 全コンポーネントの外部ヘルスプローブ
./bin/down.sh        # 全停止(成果物は volume に残る)
```

## ホスト開発ループ(サーバを変更しているとき)

スタックを一度立てたら、開発中のサーバをホストで動かし、同じ worker と S3 に
向ける。worker の URL は `./bin/up.sh` が表示する
...
次回の `up.sh` が volume と bucket を作り直す。SeaweedFS の状態自体が怪しいとき
