...
**1 プロファイル = 1 ワーカーアクター**を `spawn` する。2 つの chromium worker を
...
chromium worker に固定されず、手が空いたワーカーが次を引く。
...
| `succeeded` / `failed` | 集合ではなくカウンタ | markComplete で加算 |

完了したタスクは**件数しか残さない**。以前は taskId の `Set` に貯めていたが
削除する経路が無く、走らせるほど伸び続けていた。結果そのものは
`CaptureResultSink` が受け取り、`GET /v1/captures/{taskId}` と
`.result.json` マニフェストはそこから読む ― [キャプチャ結果](/ja/capture-results/)を参照。

カウンタを 2 本に分けているのは意図的で、`markComplete` は**成功時にも
リトライ上限を使い切った最終失敗時にも**呼ばれる。1 本の「completed」では
「撮れた数」ではなく「もう扱わない数」になってしまう。

`/v1/status` はキューの数・両カウンタ・`peekPending`(先頭 N 件)で全体像を出す。
