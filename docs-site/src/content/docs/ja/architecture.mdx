...
```bash
# docker-compose.yml が browserhive コンテナに焼き込む env(静的なプラットフォーム
# DNS 名 — IP 収集はどこにも無い)
...
flowchart TD
  ...
  UP["container-compose up<br>(docker-compose.yml・<br>名前ベース配線)"] -.->|orchestrates| BH
```

*図5: 全コンテナは Apple Container の VM。配線は名前ベース:
一度きりの `container system dns create browserhive` ドメインにより、
コンテナ同士(そしてホスト)は `<service>.browserhive` で相互解決する。
オーケストレータ側の readiness プローブは存在しない — S3 の bucket 初期化は
seaweedfs の entrypoint 内、worker の membership は browserhive が DNS から
発見し(居ない worker は除外・再試行しない)、存在するが CDP が落ちている
worker だけを再試行する(degraded モード・上限つきバックオフ)、待機は
利用側(curl / E2E globalSetup)の仕事。ホストに公開されるのは browserhive の 8080 のみ。*
...
| 関心事 | ファイル | 中身 |
|--------|----------|------|
| プロセス起点 | `bin/main.ts` | `main()`: CLI parse → startServer → SIGINT/TERM |
| CLI / 設定 | `src/cli/server-cli.ts` | `parseCliOptions`: argv + env → 設定 |
| 合成ルート | `src/bootstrap.ts` | `startServer`: Coordinator + HttpServer 構築 |
| HTTP サーバ | `src/http/http-server.ts` | `HttpServer`: Fastify/Ajv/Problem/ルート登録 |
...
| 親 machine | `src/capture/coordinator-machine.ts` | [`coordinatorMachine`](/terminology/#g-coordinatorMachine): `created→active{running/degraded}⇄reconciling→terminated` |
| membership 発見 | `src/discovery/` | `WorkerRegistry`(Static/Dns): DNS から member を解決し変化を発行 → `reconciling` |
...
| キュー | `src/capture/task-queue.ts` | [`TaskQueue`](/terminology/#g-TaskQueue): FIFO + processing Set + succeeded/failed カウンタ |
...
| デプロイ | `docker-compose.yml` / `Dockerfile.prod` | container-compose 駆動の Apple Container(S3 + worker×1〜3 + 本体) |
