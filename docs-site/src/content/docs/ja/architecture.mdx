...
```bash
# bin/stack.sh up が browserhive コンテナに焼き込む env(IP は起動時に収集)
...
flowchart TD
  ...
  UP["bin/stack.sh up<br>(bucket 初期化 one-shot・<br>IP 収集 → env 焼き込み)"] -.->|orchestrates| BH
...
compose のサービス名 DNS と `depends_on` は `bin/stack.sh up` の「逐次起動 + 外部プローブ
...
| デプロイ | `bin/stack.sh up` / `Dockerfile.prod` | Apple Container 4 コンテナ構成(S3 + worker×N + 本体) |
