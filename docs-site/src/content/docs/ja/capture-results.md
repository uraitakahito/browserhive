...
`settings` は**要求ではなく実際に効いた値**である。`cache` も `archiveMode` も
`viewport` も、リクエストが指定しなければサーバ既定が使われる ——
指定しなかった事実ではなく、**その結果何で撮られたか**が残る。
「指定していないのに値がある」のはそのためで、それが目的である。

`devicePixelRatios` が配列なのは `multipass` が 2 パス走るため。
1 つの数を書くと、`_2x` の画像が入っているアーカイブについて
「入っていないはず」と読ませてしまう。

`behaviors` は**実際に走ったもの**（`enabled ∩ isMatch()`）。
site behavior は `enabled` に載らないので、設定を写すと取りこぼす。
