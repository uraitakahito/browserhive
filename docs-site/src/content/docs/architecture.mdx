...
→ **browserhive** (this repo — the capture server) → **chromium-server-docker**
...
```bash
# The env bin/up.sh bakes into the browserhive container (IPs collected at startup)
...
flowchart TD
  ...
  UP["bin/up.sh<br>(one-shot bucket init ·<br>collect IPs → bake env)"] -.->|orchestrates| BH
  UP -.->|probe /cluster/status| SW
  UP -.->|probe /json/version| C1
  UP -.->|probe /json/version| C2
```

*Fig. 5: Every container is an Apple Container VM (mutual IP-direct on
192.168.64.0/24). Compose's service-name DNS and `depends_on` are replaced
by `bin/up.sh`'s "sequential startup + external probes (Chromium
`/json/version`, SeaweedFS `/cluster/status`) + IP collection → env baking".
Only browserhive's 8080 is published to the host.*
...
|---------|------|----------|
| Process entry | `bin/server.ts` | `main()`: CLI parse → startServer → SIGINT/TERM |
| Composition root | `src/cli/server-cli.ts` | `startServer`: builds Coordinator + HttpServer |
| HTTP server | `src/http/server.ts` | `HttpServer`: Fastify/Ajv/Problem/route registration |
...
| Deployment | `bin/up.sh` / `Dockerfile.prod` | Apple Container, 4 containers (S3 + worker×N + server) |
