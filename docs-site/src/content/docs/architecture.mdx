...
flowchart LR
  ...
  W1 -->|CDP| CR1["chromium-server-1<br>:9222"]
  W2 -->|CDP| CR2["chromium-server-2<br>:9222"]
...
(この repo・キャプチャサーバ)→ **chromium-server-docker**(CDP で操作される Chromium
...
work-stealing で取り出す(2 chromium-server → 2 ワーカー)。
...
```bash
# compose.prod.yaml — browserhive サービス env
BROWSERHIVE_BROWSER_URLS=http://chromium-server-1:9222,http://chromium-server-2:9222
BROWSERHIVE_S3_ENDPOINT=http://seaweedfs:8333
...
```mermaid
flowchart TD
  HOST["host :8080"] --> BH["browserhive-prod<br>Fastify :8080<br>node dist/bin/server.js"]
  BH -->|"CDP :9222"| C1["chromium-server-1"]
  BH -->|"CDP :9222"| C2["chromium-server-2"]
  BH -->|"S3 PutObject :8333<br>path-style"| SW["seaweedfs<br>S3 :8333 / Filer :8888"]
  INIT["seaweedfs-init<br>(bucket 作成して終了)"] -.->|completed| BH
  SW -.->|healthy| INIT
  C1 -.->|healthy| BH
  C2 -.->|healthy| BH
```

*図5: 単一ブリッジネットワーク `chromium-network`。コンテナ名 DNS で接続。
`depends_on … service_healthy / service_completed_successfully` で起動順を保証
(Chromium `/json/version`、SeaweedFS `/cluster/status`)。*

:::note[S3 アドレッシング]
バンドルされた SeaweedFS は仮想ホスト形式(`browserhive.seaweedfs:8333`)を解決できないため
**path-style**(`seaweedfs:8333/browserhive/<key>`)を強制。`BROWSERHIVE_S3_ENDPOINT` を AWS/R2 に
...
| デプロイ | `compose.prod.yaml` / `Dockerfile.prod` | 4 コンテナ構成 |
