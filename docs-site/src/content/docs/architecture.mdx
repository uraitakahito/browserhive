...
```bash
# The env bin/stack.sh up bakes into the browserhive container (IPs collected at startup)
...
flowchart TD
  ...
  UP["bin/stack.sh up<br>(one-shot bucket init ·<br>collect IPs → bake env)"] -.->|orchestrates| BH
...
by `bin/stack.sh up`'s "sequential startup + external probes (Chromium
...
| Deployment | `bin/stack.sh up` / `Dockerfile.prod` | Apple Container, 4 containers (S3 + worker×N + server) |
